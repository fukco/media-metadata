...
type NCTG struct {
	...
	ExposureCompensation *common.Rational
	MeteringMode         string
...
func ProcessNCTG(nctgBox *box.NCTG) (*NCTG, error) {
	nctg := &NCTG{}
	for _, tag := range nctgBox.Tags {
		if tagName, ok := nctgNameMap[tag.ID]; ok {
			if exif.DataType(tag.DataFormat) == exif.DTAscii {
				refValue := reflect.ValueOf(nctg).Elem()
				refValue.FieldByName(tagName).SetString(strings.TrimSpace(string(bytes.TrimRight(tag.Data, "\x00"))))
			} else if exif.DataType(tag.DataFormat) == exif.DTLong {
				switch tag.ID {
				case 0x13:
					nctg.FrameCount = int(binary.BigEndian.Uint32(tag.Data[:4]))
				default:
					refValue := reflect.ValueOf(nctg).Elem()
					if len(tag.Data) == 4 {
						refValue.FieldByName(tagName).SetInt(int64(binary.BigEndian.Uint32(tag.Data)))
					}
				}
			} else if exif.DataType(tag.DataFormat) == exif.DTByte {
				switch tag.ID {
				case 0x2000087:
					nctg.FlashMode = flashModeMap[tag.Data[0]]
				}
			} else if exif.DataType(tag.DataFormat) == exif.DTShort {
				switch tag.ID {
				case 0x1013:
					if binary.BigEndian.Uint16(tag.Data[:2]) == 0 {
						nctg.ElectronicVR = "off"
					} else if binary.BigEndian.Uint16(tag.Data[:2]) == 1 {
						nctg.ElectronicVR = "on"
					}
				case 0x1108822:
					nctg.ExposureProgram = exposureProgramMap[binary.BigEndian.Uint16(tag.Data)]
				case 0x1109207:
					nctg.MeteringMode = meteringModeMap[binary.BigEndian.Uint16(tag.Data)]
				case 0x2000022:
					nctg.ActiveDLighting = activeDLightingMap[binary.BigEndian.Uint16(tag.Data)]
				case 0x200001b:
					nctg.CropHiSpeed = cropHiSpeedMap[binary.BigEndian.Uint16(tag.Data[:2])]
					if len(tag.Data) == 14 {
						nctg.CropHiSpeed = fmt.Sprintf("%s (%dx%d cropped to %dx%d at pixel %d,%d)",
							nctg.CropHiSpeed, binary.BigEndian.Uint16(tag.Data[2:4]), binary.BigEndian.Uint16(tag.Data[4:6]),
							binary.BigEndian.Uint16(tag.Data[6:8]), binary.BigEndian.Uint16(tag.Data[8:10]),
							binary.BigEndian.Uint16(tag.Data[10:12]), binary.BigEndian.Uint16(tag.Data[12:14]))
					}
				case 0x20000b1:
					nctg.HighISONoiseReduction = highISONoiseReductionMap[binary.BigEndian.Uint16(tag.Data)]
				default:
					refValue := reflect.ValueOf(nctg).Elem()
					if len(tag.Data) == 2 {
						refValue.FieldByName(tagName).SetInt(int64(binary.BigEndian.Uint16(tag.Data)))
					}
				}
			} else if exif.DataType(tag.DataFormat) == exif.DTRational {
				switch tag.ID {
				case 0x110a432:
					nctg.LensInfo = lensInfoFormat(tag.Data)
				case 0x2000084:
					nctg.Lens = lensInfoFormat(tag.Data)
				default:
					refValue := reflect.ValueOf(nctg).Elem()
					if len(tag.Data) == 8 {
						uFraction := &common.UFraction{
							Numerator:   binary.BigEndian.Uint32(tag.Data[:4]),
							Denominator: binary.BigEndian.Uint32(tag.Data[4:8]),
						}
						refValue.FieldByName(tagName).Set(reflect.ValueOf(uFraction))
					}
				}
			} else if exif.DataType(tag.DataFormat) == exif.DTSRational {
				if tag.ID == 0x1109204 {
					refValue := reflect.ValueOf(nctg).Elem()
					if len(tag.Data) == 8 {
						fraction := &common.Rational{
							Numerator:   int32(binary.BigEndian.Uint32(tag.Data[:4])),
							Denominator: int32(binary.BigEndian.Uint32(tag.Data[4:8])),
						}
						refValue.FieldByName(tagName).Set(reflect.ValueOf(fraction))
					}
				}
			} else if exif.DataType(tag.DataFormat) == exif.DTUndefined {
				switch tag.ID {
				case 0x200001f:
					nctg.VRInfoVersion = string(tag.Data[:4])
					if tag.Data[4] == 0 {
						nctg.VibrationReduction = "n/a"
					} else if tag.Data[4] == 1 {
						nctg.VibrationReduction = "On"
					} else if tag.Data[4] == 2 {
						nctg.VibrationReduction = "Off"
					}
					if strings.HasPrefix(nctg.Model, "NIKON Z") {
						if tag.Data[6] == 0 {
							nctg.VRMode = "n/a"
						} else if tag.Data[6] == 1 {
							nctg.VRMode = "Sport"
						} else if tag.Data[6] == 3 {
							nctg.VRMode = "Normal"
						}
					}
				case 0x2000023:
					if !bytes.HasPrefix(tag.Data, []byte("03")) {
						continue
					}
					pictureControlData := &PictureControlData{}
					nctg.PictureControlData = pictureControlData
					pictureControlData.PictureControlVersion = string(tag.Data[:4])
					pictureControlData.PictureControlName = string(bytes.TrimRight(tag.Data[8:28], "\x00"))
					pictureControlData.PictureControlBase = string(bytes.TrimRight(tag.Data[28:48], "\x00"))
					if tag.Data[54] == 0 {
						pictureControlData.PictureControlAdjust = "Default Settings"
					} else if tag.Data[54] == 1 {
						pictureControlData.PictureControlAdjust = "Quick Adjust"
					} else if tag.Data[54] == 2 {
						pictureControlData.PictureControlAdjust = "Full Control"
					}
					//TODO
					//if tag.Data[55] == 0 {
					//	pictureControlData.PictureControlQuickAdjust = "Normal"
					//} else if tag.Data[55] == 0x7f {
				...
				case 0x2000025:
					...
					isoInfo.ISO = fmt.Sprintf("%.0f", 100*math.Pow(2, float64(tag.Data[0])/12-5))
					isoInfo.ISOExpansion = isoExpansionMap[binary.BigEndian.Uint16(tag.Data[4:6])]
					isoInfo.ISO2 = fmt.Sprintf("%.0f", 100*math.Pow(2, float64(tag.Data[0])/12-5))
					isoInfo.ISOExpansion2 = isoExpansionMap[binary.BigEndian.Uint16(tag.Data[10:12])]
				case 0x20000b7:
					//TODO not finished
					afInfo2 := &AFInfo2{}
					nctg.AFInfo2 = afInfo2
					afInfo2.AFInfo2Version = string(tag.Data[:4])
					if tag.Data[4] == 0 {
					...
					if tag.Data[4] == 0 {
						afInfo2.AFAreaMode = afAreaModeOnContrastDetectAFOffMap[tag.Data[5]]
					} else {
						afInfo2.AFAreaMode = afAreaModeOnContrastDetectAFOnMap[tag.Data[5]]
					}
				case 0x2000098:
					if strings.HasPrefix(string(tag.Data[:4]), "080") {
						lensData := &LensData{}
						nctg.LensData = lensData
						lensData.LensDataVersion = string(tag.Data[:4])
						//TODO
					}
				default:
					println(tagName)
